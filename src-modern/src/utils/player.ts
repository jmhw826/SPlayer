...
import { songUrl, songLyric, songChorus, getMusicNumUrl } from "@/api/song";
...
class Player {
  ...
  private async getUnlockSongUrl(songData: SongType): Promise<string | null> {
    try {
      ...
      // 尝试解锁
      const neteaseUrl = await getMusicNumUrl(songId);
      if (neteaseUrl.code === 200 && neteaseUrl.data?.url) {
        return neteaseUrl.data.url;
      }
      return null;
    } catch (error) {
  ...
  private playerEvent(
    ...
    // 播放
    this.player.on("play", () => {
      window.document.title = this.getPlayerInfo() || "云音乐";
      // ipc
      if (isElectron) {
    ...
    this.player.on("pause", () => {
      if (!isElectron) window.document.title = "云音乐";
      // ipc
  ...
  async initPlayer(autoPlay: boolean = true, seek: number = 0) {
    ...
    try {
      ...
      // 在线歌曲
        ...
        // 正常播放地址
        if (url) {
          if (songData?.freeTrialInfo == "unblock") {
