    ...
    <div class="set-list">
      ...
      <n-card v-if="!isElectron" class="set-item">
        <div class="label">
          ...
          <n-text class="tip" :depth="3">是否在非会员状态下播放试听歌曲; 注意: 与音乐解锁选项冲突</n-text>
        </div>
      ...
      <n-card v-if="!isElectron" class="set-item">
        <div class="label">
          ...
          <n-text class="tip" :depth="3">在无法正常播放时进行替换，可能会与原曲不符; 注意: 与播放试听选项冲突</n-text>
        </div>
...
const playDeviceChange = (deviceId: string, option: SelectOption) => {
  if (settingStore.showSpectrums) {
    window.$dialog.warning({
      title: "音频通道占用",
      content:
        "由于系统限制，切换音频输出设备会导致音乐频谱失效，将会关闭音乐频谱，并将于网站刷新后生效，是否继续？",
      positiveText: "继续",
      negativeText: "取消",
      closeOnEsc: false,
      closable: false,
